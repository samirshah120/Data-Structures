...
public class MazeTest extends JFrame implements GridColors {
 ...
 //       Blob aBlob = new Blob(aGrid);
    ...
    public void solve() {
        ...
        int index = 0;
        while(index < m.findAllMazePaths(0,0).size()) //Displays all paths
        {
           System.out.println("Path " +index +
           m.findAllMazePaths(0,0).get(index));
           index++;
        }
        if(m.findAllMazePaths(0,0).size() == 0) //if path size is 0
        {
        	System.out.println(m.findAllMazePaths(0,0));
        }
        
        ArrayList<PairInt> list = m.findMazePathMin(0,0); //Displays minimum path
        System.out.println("Min Path"+list);
